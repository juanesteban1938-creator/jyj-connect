      ...
      const response = await fetch('/api/send-invoice', {
        ...
        body: JSON.stringify({
          ...
          servicioData: {
            ...
            nit: pend.nitCliente,
            fecha: pend.fechaServicio || pend.fecha?.toDate?.()?.toISOString() || new Date().toISOString(),
            valor: Number(pend.valorTotal) || Number(pend.valorPago) || 0,
            origen: pend.origen || 'Consultar Panel',
            destino: pend.destino || 'Consultar Panel',
            vehiculo: pend.vehiculo || 'Asignado',
            conductor: pend.conductor || 'Asignado'
          }
        })
      });
    ...
    <div className="space-y-8 pb-12">
      <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <div className="flex items-center gap-3">
            ...
            <Badge className="bg-orange-500 text-white font-black px-3 py-1 rounded-lg">{statsDia.count} HOY</Badge>
          </div>
          <p className="text-slate-500 text-sm font-medium mt-1">Supervisión de conciliaciones automáticas procesadas por Nova.</p>
        </div>
        <div className="flex items-center gap-3">
          <Button variant="outline" onClick={handleReconcile} disabled={isProcessing === 'reconcile'} className="font-black text-[10px] uppercase border-orange-200 text-orange-600 hover:bg-orange-50 h-11">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card className="border-none shadow-sm bg-white overflow-hidden group">
          <CardContent className="p-6">
            <div className="flex justify-between items-start mb-4">
              <div className="p-2.5 rounded-xl bg-emerald-50 text-emerald-600"><DollarSign className="h-5 w-5" /></div>
        ...
        <Card className="border-none shadow-sm bg-white overflow-hidden">
          <CardContent className="p-6">
            <div className="flex justify-between items-start mb-4">
              <div className="p-2.5 rounded-xl bg-blue-50 text-blue-600"><CreditCard className="h-5 w-5" /></div>
        ...
        <Card className="border-none shadow-sm bg-white overflow-hidden">
          <CardContent className="p-6">
            <div className="flex justify-between items-start mb-4"><div className="p-2.5 rounded-xl bg-orange-50 text-orange-600"><TrendingUp className="h-5 w-5" /></div></div>
        ...
        <Card className="border-none shadow-sm bg-white overflow-hidden">
          <CardContent className="p-6">
            <div className="flex justify-between items-start mb-4"><div className="p-2.5 rounded-xl bg-indigo-50 text-indigo-600"><CheckCircle2 className="h-5 w-5" /></div></div>
      ...
      <Card className="rounded-3xl shadow-sm border-none overflow-hidden bg-white">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-3">
              <div className="relative w-full sm:w-64">
                ...
                <Input placeholder="Cliente o Consecutivo..." className="pl-9 h-10 bg-white border-slate-200 rounded-xl text-xs focus:ring-orange-500" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
              </div>
              <div className="flex gap-1 bg-white p-1 rounded-xl border border-slate-200 w-full sm:w-auto">
                {['Todos', 'Pagado', 'Anticipo'].map(est => (
                  <button key={est} onClick={() => setFilterEstado(est)} className={cn("px-4 py-1.5 text-[10px] font-black uppercase rounded-lg transition-all flex-1 sm:flex-none", filterEstado === est ? "bg-orange-500 text-white shadow-md shadow-orange-500/20" : "text-slate-400 hover:text-slate-600")}>{est}</button>
                ))}
              </div>
            </div>
          </div>
        ...
        <div className="overflow-x-auto w-full">
          <Table className="min-w-full">
            ...
            <TableBody>
              {loadingPagos ? (
                <TableRow><TableCell colSpan={8} className="p-20 text-center"><Loader2 className="h-8 w-8 animate-spin text-orange-500 mx-auto mb-4" /><p className="text-xs font-black uppercase text-slate-400 tracking-widest">Sincronizando cobros...</p></TableCell></TableRow>
              ) : filteredPagos.map((p) => {
                const fecha = p.fecha instanceof Timestamp ? p.fecha.toDate() : new Date(p.fecha);
                return (
                  <TableRow key={p.id} className="hover:bg-slate-50/50 transition-colors border-b border-slate-50 last:border-0">
                    <TableCell className="p-5"><div className="flex flex-col"><span className="text-xs font-black text-slate-700">{format(fecha, 'dd MMM yyyy', { locale: es }).toUpperCase()}</span><span className="text-[10px] font-bold text-slate-400">{format(fecha, 'HH:mm')}</span></div></TableCell>
                    <TableCell className="p-5"><Badge variant="outline" className="font-black text-[10px] border-orange-100 text-orange-600 bg-orange-50/30">{p.consecutivo}</Badge></TableCell>
                    <TableCell className="p-5"><div className="flex flex-col max-w-[180px]"><span className="text-xs font-black text-slate-800 uppercase truncate">{p.clienteNombre}</span><span className="text-[10px] font-bold text-slate-400">{p.telefonoCliente}</span></div></TableCell>
                    <TableCell className="p-5 text-right"><span className="text-sm font-black text-emerald-600">{currencyFormatter.format(p.valorPago)}</span></TableCell>
                    <TableCell className="p-5"><div className="flex items-center gap-3"><div className="p-2 rounded-lg bg-slate-50 text-slate-400"><Landmark className="h-3.5 w-3.5" /></div><div className="flex flex-col"><span className="text-[10px] font-black text-slate-700 uppercase tracking-tighter">{p.numeroTransaccion}</span><span className="text-[9px] font-bold text-slate-400 uppercase">{p.bancoOrigen} ➔ {p.bancoDestino}</span></div></div></TableCell>
                    <TableCell className="p-5 text-right"><div className="flex flex-col items-end"><span className="text-[10px] font-bold text-slate-400 line-through decoration-slate-300">{currencyFormatter.format(p.saldoAnterior)}</span><div className="flex items-center gap-1.5 text-xs font-black text-slate-800"><ArrowUpRight className="h-3 w-3 text-orange-500" />{currencyFormatter.format(p.saldoNuevo)}</div></div></TableCell>
                    <TableCell className="p-5 text-center"><Badge className={cn("text-[9px] font-black uppercase px-2 py-0.5 rounded-md", p.estadoPago === 'Pagado' ? "bg-emerald-50 text-emerald-600 border-emerald-100" : "bg-orange-50 text-orange-600 border-orange-100")}>{p.estadoPago}</Badge></TableCell>
                    <TableCell className="p-5 text-center">
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-8 w-8 rounded-lg" disabled={isProcessing === p.id}>{isProcessing === p.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <MoreHorizontal className="h-4 w-4" />}</Button></DropdownMenuTrigger>
                        <DropdownMenuContent align="end" className="w-56 p-2 rounded-xl shadow-xl">
                          <DropdownMenuItem className="rounded-lg font-bold text-xs py-2.5" onClick={() => router.push(`/dashboard/servicios?consecutivo=${p.consecutivo}`)}><Eye className="mr-2 h-4 w-4 text-slate-400" /> Ver Servicio</DropdownMenuItem>
                          <DropdownMenuItem className="rounded-lg font-bold text-xs py-2.5" onClick={() => { setEditingPago({...p}); setIsEditOpen(true); }}><Edit className="mr-2 h-4 w-4 text-slate-400" /> Editar Registro</DropdownMenuItem>
                          <DropdownMenuItem className="text-red-600 rounded-lg font-bold text-xs py-2.5 bg-red-50/50 mt-1" onClick={() => handleDeletePago(p.id)}><Trash2 className="mr-2 h-4 w-4" /> Eliminar Registro</DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </TableCell>
                  </TableRow>
      ...
      <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
        <DialogContent className="sm:max-w-[425px] rounded-3xl border-none shadow-2xl">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-xl font-black uppercase tracking-tight"><div className="p-2 rounded-xl bg-orange-500 text-white"><Edit className="h-5 w-5" /></div>Editar Registro</DialogTitle>
          </DialogHeader>
          {editingPago && (
            <form onSubmit={handleSaveEditPago} className="space-y-5 pt-4">
              <div className="space-y-2"><Label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Valor Recaudado</Label><div className="relative"><DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" /><Input type="number" className="pl-9 h-12 bg-slate-50 border-slate-200 rounded-xl font-bold" value={editingPago.valorPago} onChange={e => setEditingPago({...editingPago, valorPago: e.target.value})} /></div></div>
              <div className="space-y-2"><Label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Referencia</Label><Input className="h-12 bg-slate-50 border-slate-200 rounded-xl font-bold" value={editingPago.numeroTransaccion} onChange={e => setEditingPago({...editingPago, numeroTransaccion: e.target.value})} /></div>
              <div className="space-y-2"><Label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Estado</Label><Select value={editingPago.estadoPago} onValueChange={val => setEditingPago({...editingPago, estadoPago: val})}><SelectTrigger className="h-12 bg-slate-50 border-slate-200 rounded-xl font-bold"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="Pagado">Pagado (Total)</SelectItem><SelectItem value="Anticipo">Anticipo (Parcial)</SelectItem></SelectContent></Select></div>
              <div className="flex justify-end gap-3 pt-6 border-t"><Button type="button" variant="ghost" onClick={() => setIsEditOpen(false)}>Cancelar</Button><Button type="submit" className="bg-orange-500 text-white font-black px-6 rounded-xl" disabled={!!isProcessing}>Guardar</Button></div>
            </form>
          )}
        </DialogContent>
      </Dialog>
